...
@Aspect @Order(-1)  // 该切面应当先于 @Transactional 执行
@Component public class DynamicDataSourceAspect {
    @Autowired private Environment env;

    @Pointcut("execution(* com.digitalbase.datasource.mapper..*(..))") public void test() {
    }
        ...
        for (Object obj : args) {
            if (obj instanceof BasicInfoBean) {
                ...
                if (DynamicDataSourceContextHolder.containDataSourceKey(baseName)) {
                    ...
                    DynamicDataSource dynamicDataSource = SpringContextHolder.getApplicationContext()
                        .getBean("dynamicDataSource", DynamicDataSource.class);
     ...
     * @param point
     */
    @After("test()") public void restoreDataSource(JoinPoint point) {
        // 将数据源置为默认数据源
        DynamicDataSourceContextHolder.clearDataSourceKey();
        System.out.println(
            "Restore TargetDataSource to [" + DynamicDataSourceContextHolder.getDataSourceKey() + "] in Method ["
                + point.getSignature() + "]");
    }
    ...
    private DataSource determineDataSource(BasicInfoBean bean) throws Exception {
        ...
        properties.put("url", "jdbc:mysql://127.0.0.1:" + bean.getPort() + "/" + bean.getBaseName()
            + "?useSSL=false&serverTimezone=Asia/Shanghai&autoReconnect=true&useUnicode=true&characterEncoding=utf8");
